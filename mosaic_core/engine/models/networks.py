...
def define_G(input_nc, output_nc, ngf, netG,
    ...
    if netG == 'resnet_9blocks':
        ...
        net = Generator(nz=nz, ngf=ngf, img_size=resolution, nc=input_nc)
    else:
        raise NotImplementedError('Generator model name [%s] is not recognized' % netG)
    return init_net(net, init_type, init_gain, gpu_ids)


def define_D(input_nc, ndf, netD, n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02, gpu_ids=[], resolution=64):
    ...
    if netD == 'basic':  # default PatchGAN classifier
        ...
        net = Discriminator(nc=input_nc, img_size=resolution, ndf=ndf)
    else:
...
class UnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, num_downs, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False):
        ...
        unet_block = UnetSkipConnectionBlock_upsample(ngf * 8, ngf * 8, input_nc=None, submodule=None, norm_layer=norm_layer, innermost=True)  # add the innermost layer
        ...
        self.model = UnetSkipConnectionBlock_upsample(output_nc, ngf, input_nc=input_nc, submodule=unet_block, outermost=True, norm_layer=norm_layer)  # add the outermost layer
...
class UnetSkipConnectionBlock_upsample(nn.Module):
    ...
    def __init__(self, outer_nc, inner_nc, input_nc=None,
        ...
        if outermost:
            ...
            #                             kernel_size=4, stride=2,
            #                             padding=1)
            upconv = nn.Sequential(*[nn.Upsample(scale_factor=2), nn.Conv2d(inner_nc * 2, outer_nc, kernel_size=3, stride=1,
                                        padding=1)])
            down = [downconv]
            up = [uprelu, upconv, nn.Tanh()]
            model = down + [submodule] + up
